...
impl<'a> JsonDocument<'a> {
    pub fn parse(source: &'a str) -> ConfigResult<Self> {
...
pub fn set_scalar_preserving(content: &str, path: &str, value: &Value) -> ConfigResult<String> {
    ...
    match resolve(root, &segments, 0) {
        ...
        None => {
            ...
            let parent = resolve(root, parents, 0).ok_or_else(|| ConfigError::PathNotFound {
                path: path.to_string(),
            })?;
            ...
            let new_key = serde_json::to_string(last).map_err(|error| ConfigError::ParseError {
                format: "JSON".to_string(),
                detail: error.to_string(),
            })?;
...
pub fn unset_preserving(content: &str, path: &str) -> ConfigResult<String> {
    ...
    let document = JsonDocument::parse(content)?;
    let root = document.root();
    let target = resolve(root, &segments, 0).ok_or_else(|| ConfigError::PathNotFound {
        path: path.to_string(),
    })?;
...
impl<'a> Parser<'a> {
    ...
    fn parse_value(&mut self, mut position: usize) -> ConfigResult<Node> {
    ...
    fn parse_object(&mut self, position: &mut usize) -> ConfigResult<NodeKind> {
        *position += 1;
    ...
    fn parse_array(&mut self, position: &mut usize) -> ConfigResult<NodeKind> {
        *position += 1;
    ...
    fn parse_string(&self, mut position: usize) -> ConfigResult<usize> {
        if self.source.get(position) != Some(&b'"') {
    ...
    fn parse_scalar(&self, mut position: usize) -> ConfigResult<usize> {
        let start = position;
        while let Some(byte) = self.source.get(position).copied() {
    ...
    fn error<T>(&self, position: usize, detail: &str) -> ConfigResult<T> {
