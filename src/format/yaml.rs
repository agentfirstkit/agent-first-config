...
pub fn set_scalar_preserving(content: &str, path: &str, value: &Value) -> ConfigResult<String> {
    let segments = crate::parse_path(path)?;
    ...
    let mut document = parse_document(content).map_err(|error| ConfigError::ParseError {
        format: "YAML".to_string(),
    ...
    let exists = load(content)
        .ok()
        .is_some_and(|loaded| crate::get_path_ref(&loaded, path, &[]).is_ok());
    if exists {
        document
            .set_value(&yaml_path, &to_noyalib_value(value)?)
            .map_err(|error| ConfigError::UnsupportedOperation {
                format: "YAML".to_string(),
                operation: "set".to_string(),
                detail: error.to_string(),
            })?;
    } else {
        let (last, parents) = segments.split_last().ok_or(ConfigError::EmptyPath)?;
...
pub fn unset_preserving(content: &str, path: &str) -> ConfigResult<String> {
    let segments = crate::parse_path(path)?;
...
pub fn append_array_item_preserving(
    ...
    let fragment =
        to_string(&to_noyalib_value(item)?).map_err(|error| ConfigError::UnsupportedOperation {
            format: "YAML".to_string(),
            operation: "add".to_string(),
            detail: error.to_string(),
        })?;
    let yaml_path = cst_path(&crate::parse_path(path)?, "add")?;
    document
...
pub fn remove_array_item_preserving(
    ...
    let yaml_path = cst_path(&crate::parse_path(path)?, "remove")?;
    document
...
fn cst_path(segments: &[String], operation: &str) -> ConfigResult<String> {
...
fn to_noyalib_value(value: &Value) -> ConfigResult<YamlValue> {
    match value {
        ...
        Value::Float(_) => Err(ConfigError::UnsupportedOperation {
            format: "YAML".to_string(),
            operation: "set".to_string(),
...
pub fn save(value: &Value) -> ConfigResult<String> {
    ...
    to_string(&yaml_val).map_err(|e| ConfigError::ParseError {
        format: "YAML".to_string(),
        detail: e.to_string(),
    })
}
...
fn our_value_to_yaml_value(v: &Value) -> ConfigResult<YamlValue> {
    match v {
        Value::Null => Ok(YamlValue::Null),
