...
pub fn from_value<T: DeserializeOwned>(value: &Value, path: &str) -> ConfigResult<T> {
    T::deserialize(ValueDeserializer(value)).map_err(|error| ConfigError::from_serde(path, error))
}

/// Serialize a typed configuration into the format-independent value view.
pub fn to_value<T: Serialize>(value: &T) -> ConfigResult<Value> {
    value
